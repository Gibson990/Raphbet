...
export const StandingsTable: React.FC<StandingsTableProps> = ({ standings }) => {
  if (standings.length === 0) {
    return <div className="text-center text-gray-500 py-10">Standings not available for this league.</div>;
  }
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm text-left text-gray-500">
        ...
        <tbody>
          {standings.map((s) => (
            <tr key={s.team.id} className="bg-white border-b hover:bg-gray-50">
              ...
              <td className="px-2 py-4 text-center font-bold text-gray-900">{s.points}</td>
            </tr>
          ))}
        </tbody>
      </table>
